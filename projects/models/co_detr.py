...
class CoDETR(BaseDetector):
    def __init__(
        ...
        if mask_head is not None:
            ...
            rcnn_train_cfg = (
                train_cfg[head_idx]
                if (train_cfg and train_cfg[head_idx] is not None)
                else None
            )
        ...
        if mim_head is not None:
            self.mim_head = build_head(mim_head)
    ...
    @property
    def with_mim(self):
        """bool: whether the detector has a MIM head"""
        return hasattr(self, "mim_head") and self.mim_head is not None
    ...
    def _mask_forward_train(self, x, sampling_results, gt_masks, img_metas):
        ...
        if hasattr(self, "mask_iou_head"):
            ...
            pos_mask_iou_pred = mask_iou_pred[
                range(mask_iou_pred.size(0)), torch.cat(pos_labels)
            ]
            mask_iou_targets = self.mask_iou_head.get_targets(
                sampling_results,
                gt_masks,
                pos_mask_pred,
                stage_mask_targets[1],
                self.rcnn_train_cfg,
            )
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        if self.with_query_head:
            detr_forward_results = self.query_head.forward_train(
        ...
        if self.with_mim:
            # print(
            #     "using features:",
            #     self.mim_head.use_backbone and self.backbone_output is not None,
            # )
            feat = (
                self.backbone_output[-1]
                if self.mim_head.use_backbone and self.backbone_output is not None
                else x[0]
            )
            recon = self.mim_head(feat, output_size=img.shape[-2:])
            target = clear_image if clear_image is not None else img
            loss_mim = self.mim_head.loss(recon, target)
            losses.update(dict(loss_mim=loss_mim))
        ...
        if hasattr(self, "mask_head"):
            ...
            for i in range(num_imgs):
                assign_result = self.bbox_assigner.assign(
                    results_list[i], gt_bboxes[i], gt_bboxes_ignore[i], gt_labels[i]
                )
            ...
            mask_results = self._mask_forward_train(
                x, sampling_results, gt_masks, img_metas
            )
        ...
        if hasattr(self, "mim_head"):
            ...
            return list(
                zip(
                    bbox_results,
                    recon,
                    clear_image if clear_image is not None else img,
                )
            )
    ...
    def simple_test_bbox_head(self, img, img_metas, proposals=None, rescale=False):
        ...
        bbox_results = [
            bbox2result(
                det_bboxes, det_labels, self.bbox_head[self.eval_index].num_classes
            )
            for det_bboxes, det_labels in results_list
        ]
        return bbox_results
            ...
            if det_bboxes.shape[0] == 0:
                ...
                for i in range(0, det_labels.shape[0], interval):
                    mask_results = self._mask_forward(
                        x, mask_rois[i : i + interval], det_labels[i : i + interval]
                    )
                    ...
                    if hasattr(self, "mask_iou_head"):
                        ...
                        mask_iou_pred = self.mask_iou_head(mask_feats, mask_pred)
                        chunk_mask_score = self.mask_iou_head.get_mask_scores(
                            mask_iou_pred,
                            det_bboxes[i : i + interval],
                            det_labels[i : i + interval],
                            return_score=True,
                        )
                    ...
                    for idx in range(len(stage_instance_preds) - 1):
                        instance_pred = (
                            stage_instance_preds[idx].squeeze(1).sigmoid() >= 0.5
                        )
                        non_boundary_mask = (
                            generate_block_target(instance_pred, boundary_width=1) != 1
                        ).unsqueeze(1)
                        non_boundary_mask = (
                            F.interpolate(
                        ...
                        stage_instance_preds[idx + 1][non_boundary_mask] = pre_pred[
                            non_boundary_mask
                        ]
                    ...
                    if hasattr(self, "mask_iou_head"):
                        ...
                        for c, segm in zip(
                            det_labels[i : i + interval], chunk_segm_result
                        ):
                            segm_result[c].append(segm)
    ...
    def simple_test(
        self, img, img_metas, proposals=None, rescale=False, clear_image=None
    ):
        """Test without augmentation."""
        ...
        return self.simple_test_query_head(
            img, img_metas, proposals, rescale, clear_image=clear_image
        )
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        results_list = self.query_head.aug_test(feats, img_metas, rescale=rescale)
        bbox_results = [
            bbox2result(det_bboxes, det_labels, self.query_head.num_classes)
            for det_bboxes, det_labels in results_list
        ]
