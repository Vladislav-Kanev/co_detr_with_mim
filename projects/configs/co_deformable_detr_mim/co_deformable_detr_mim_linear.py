...
model = dict(
    ...
    rpn_head=dict(
        ...
        loss_cls=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=6.0),
        ...
        dict(
            ...
            bbox_head=dict(
                ...
                loss_cls=dict(
                    ...
                    use_sigmoid=False,
                    loss_weight=6.0),
                loss_bbox=dict(type='GIoULoss', loss_weight=60.0)))
    ],
    bbox_head=[
        dict(
            type='CoATSSHead',
...
albu_train_transforms = [
    # dict(type='MotionBlur', blur_limit=5, p=1),
    # dict(type='GaussNoise', var_limit=(2.0, 5.0), p=1),
    dict(type='GridDropout', ratio=0.5, holes_number_x=5,
         holes_number_y=5, shift_x=0, shift_y=0, p=1.0),

    # ✅ Cutout (имитация случайной маскировки)
    dict(type='Cutout', num_holes=32, max_h_size=16,
