...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if "clear_image" in results:
            clear_image = results["clear_image"]
            if self.img_to_float is True and img.dtype == np.uint8:
                clear_image = clear_image.astype(np.float32)

            if len(clear_image.shape) < 3:
                clear_image = np.expand_dims(clear_image, -1)

            clear_image = np.ascontiguousarray(clear_image.transpose(2, 0, 1))
            results["clear_image"] = DC(
                to_tensor(clear_image),
