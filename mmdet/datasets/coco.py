...
class CocoDataset(CustomDataset):
    ...
    def _mim2json(self, results):
        """Convert detection + MIM results to COCO json-style with PSNR metrics."""
        bbox_json_results = []
        psnr_results = []

        for idx in range(len(self)):
            img_id = self.img_ids[idx]
            det, restored, clear = results[idx]
            ...
            # compute psnr (example assumes single image per idx)
            psnr_val = psnr(restored, clear).tolist()
            psnr_results.append(dict(image_id=img_id, psnr=psnr_val))

        return bbox_json_results, psnr_results
    ...
    def results2json(self, results, outfile_prefix):
        ...
        if isinstance(results[0], list):
            ...
            json_results, psnr_results = self._mim2json(results)
            result_files["bbox"] = f"{outfile_prefix}.bbox.json"
            result_files["proposal"] = f"{outfile_prefix}.bbox.json"
            result_files["psnr"] = f"{outfile_prefix}.psnr.json"
            mmcv.dump(json_results, result_files["bbox"])
            mmcv.dump(psnr_results, result_files["psnr"])
